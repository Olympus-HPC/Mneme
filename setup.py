...
class CMakeBuild(build_ext):
    ...
    def write_python_config(self):
        """Write a Python file with absolute native library paths."""

        native_lib_dir = Path(self.install_dir) / "lib64"

        core_lib = native_lib_dir / (
            "libmneme.dylib" if sys.platform == "darwin" else "libmneme.so"
        )
        profile_lib = native_lib_dir / (
            "libmneme_profile.dylib"
            if sys.platform == "darwin"
            else "libmneme_profile.so"
        )
        record_lib = native_lib_dir / (
            "librecord.dylib" if sys.platform == "darwin" else "librecord.so"
        )

        config = (
            "# Auto-generated by setup.py\n"
            + f"MNEME_CORE_LIB = '{core_lib}'\n"
            + f"MNEME_PROFILE_LIB = '{profile_lib}'\n"
            + f"MNEME_RECORD_LIB = '{record_lib}'\n"
            + f"MNEME_CONFIG_FILE = '{self.config_json}'\n"
        )
