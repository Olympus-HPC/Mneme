...
- `SourceFile`: Absolute path of the file that defines the kernel, taken from line-table debug info. Falls back to the kernel's translation unit when the recorded module has no debug info, and is omitted for LTO-linked (HIP RDC) builds without debug info.
- `SourceLine`, `SourceEndLine`: First and last line of the kernel definition, inclusive and 1-based. The end line is the last line that generated code. Present only when the recorded module carries line-table debug info.
- `SourceMD5`: MD5 checksum of `SourceFile`. It is the checksum recorded at compile time when debug info provides one, so it identifies the exact revision the line numbers refer to.
- `SourceCopy`: Copy of `SourceFile` in the record directory, written only with `--copy-source`. The copy is skipped when the file no longer matches `SourceMD5`.
