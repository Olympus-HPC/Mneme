...
- `CaptureMode`: `full` when the snapshots hold every tracked allocation, `reachable` when they hold only the allocations reachable from kernel arguments (see `--capture-mode`). Recordings written before this field existed are `full`.
...
The prologue file contains a serialized snapshot of GPU memory, global
variables, metadata, and kernel argument mappings. Which allocations it
holds depends on `--capture-mode`; see [Record → Choosing what to
capture](record.md#choosing-what-to-capture). By default, the epilogue
file is stored as a binary diff against the matching prologue.
