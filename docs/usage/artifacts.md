...
- `SourceFile`: Absolute path of the file that defines the kernel, taken from line-table debug info. It is omitted, together with the other `Source*` fields, when the recorded module has no debug info; the recorder then warns and suggests compiling with `-gline-tables-only`.
- `SourceLine`, `SourceEndLine`: First and last line of the kernel definition, inclusive and 1-based. The end line is the last line that generated code. Both are present whenever `SourceFile` is.
