...
- `SourceFile`: Absolute path of the translation unit that defined the kernel, present when known. Kernels defined in headers or instantiated from templates report the translation unit that instantiated them; kernels from LTO-linked (HIP RDC) builds omit this field.
- `SourceCopy`, `SourceMD5`: Present when recording with `--copy-source`. `SourceCopy` names the copy of `SourceFile` stored in the record directory as `RecordedSource_<md5>_<basename>`, and `SourceMD5` is the MD5 checksum of its contents.
