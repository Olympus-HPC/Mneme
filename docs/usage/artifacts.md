...
- `SourceFile`: Absolute path of the kernel's translation unit. Omitted for LTO-linked (HIP RDC) builds.
- `SourceCopy`, `SourceMD5`: Copy of `SourceFile` in the record directory and its MD5 checksum. Written only with `--copy-source`.
