...
void serialize(llvm::raw_ostream &OS, const Metadata& Md){ 
  OS.write(reinterpret_cast<const char*>(&Md.builtin), sizeof(std::underlying_type_t<BuiltinDType>));
  OS.write(reinterpret_cast<const char*>(&Md.threshold), sizeof(double));
  OS.write(reinterpret_cast<const char*>(&Md.threshold_kind), sizeof(ThresholdKind));
