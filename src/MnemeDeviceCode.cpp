...
namespace mneme {
  namespace dev {
    ...
    template <class T>
      MNEME_DEV T mneme_abs(T x) {
        if constexpr (std::is_floating_point_v<T>) return x < T(0) ? -x : x;
        else return x < T(0) ? -x : x; // for signed ints
      }

    template <class T>
      MNEME_DEV T mneme_max(T a, T b) { return a > b ? a : b; }

    template <class T>
      MNEME_DEV T mneme_min(T a, T b) { return a < b ? a : b; }

    // Promote integer diffs to double for aggregation.
    template <class T>
      MNEME_DEV double diff_as_double(T a, T b) {
    ...
    MNEME_DEV uint32_t float_to_ordered_uint(float f) {
      ...
      if (u & 0x80000000u) u = 0x80000000u - u;
      return u;
    }
    ...
    MNEME_DEV uint64_t double_to_ordered_uint(double d) {
      uint64_t u;
      memcpy(&u, &d, sizeof(u));
      if (u & 0x8000000000000000ull) u = 0x8000000000000000ull - u;
      return u;
    }

    template <class T>
      MNEME_DEV uint64_t ulp_distance(T a, T b) {
    ...
    //
    template <class T>
      MNEME_DEV double element_error(T a, T b, ThresholdKind kind, double rel_eps = 1e-12) {
        const double abs_err = dabs(diff_as_double(a, b));
        if (kind == ThresholdKind::Absolute) return abs_err;
    ...
    MNEME_DEV double atomicMaxDouble(double* addr, double val) {
      ...
      double out; memcpy(&out, &old, sizeof(out));
      return out;
      ...
      do {
        ...
        unsigned long long next;
        memcpy(&next, &val, sizeof(next));
        old = (unsigned long long)atomicCAS((unsigned long long*)ull, assumed, next);
      } while (assumed != old);
      double out; memcpy(&out, &old, sizeof(out));
      return out;
#endif
    ...
    MNEME_DEV int atomicMinInt(int* addr, int val) {
      return atomicMin(addr, val);
    }

    template <class T>
      __global__ void compare_builtin_kernel(const T* __restrict__ a,
          const T* __restrict__ b,
          std::size_t NumElements,
          ThresholdKind thr_kind,
          Norm norm,
          double threshold,
          CompareResult* out) {
...
namespace mneme{
  CompareResult compareDeviceBlobs(const char *Blob1, const char *Blob2, uint64_t NumBytes, Metadata Md) {
    ...
    EC = DeviceVendorTraits::DeviceErrorCheck(DeviceVendorTraits::DeviceCopy(Res, &Init, sizeof(Init),
          DeviceVendorTraits::MemcpyHostToDeviceKind()));
    if (EC)
    ...
    uint64_t NumBlocks = std::min( (NumElements + NUM_THREADS_PER_BLOCK - 1)/NUM_THREADS_PER_BLOCK,
        (uint64_t)8192 );   // DN -- you could also set this to something like SMs*20 to be more device dependent


    switch (Md.builtin) {
      case BuiltinDType::U8:
        mneme::dev::compare_builtin_kernel<<<NumBlocks, NUM_THREADS_PER_BLOCK>>>(reinterpret_cast<const uint8_t*>(Blob1), 
            reinterpret_cast<const uint8_t*>(Blob2), 
            NumElements, 
            Md.threshold_kind, Md.norm, Md.threshold, Res);
        break;
      case BuiltinDType::I8:
        mneme::dev::compare_builtin_kernel<<<NumBlocks, NUM_THREADS_PER_BLOCK>>>(reinterpret_cast<const int8_t*>(Blob1), 
            reinterpret_cast<const int8_t*>(Blob2), 
            NumElements, 
            Md.threshold_kind, Md.norm, Md.threshold, Res);
        break;

      case BuiltinDType::U16:
        mneme::dev::compare_builtin_kernel<<<NumBlocks, NUM_THREADS_PER_BLOCK>>>(reinterpret_cast<const uint16_t*>(Blob1), 
            reinterpret_cast<const uint16_t*>(Blob2), 
            NumElements, 
            Md.threshold_kind, Md.norm, Md.threshold, Res);
        break;

      case BuiltinDType::I16:
        mneme::dev::compare_builtin_kernel<<<NumBlocks, NUM_THREADS_PER_BLOCK>>>(reinterpret_cast<const int16_t*>(Blob1), 
            reinterpret_cast<const int16_t*>(Blob2), 
            NumElements, 
            Md.threshold_kind, Md.norm, Md.threshold, Res);
        break;

      case BuiltinDType::U32:
        mneme::dev::compare_builtin_kernel<<<NumBlocks, NUM_THREADS_PER_BLOCK>>>(reinterpret_cast<const uint32_t*>(Blob1), 
            reinterpret_cast<const uint32_t*>(Blob2), 
            NumElements, 
            Md.threshold_kind, Md.norm, Md.threshold, Res);
        break;

      case BuiltinDType::I32:
        mneme::dev::compare_builtin_kernel<<<NumBlocks, NUM_THREADS_PER_BLOCK>>>(reinterpret_cast<const int32_t*>(Blob1), 
            reinterpret_cast<const int32_t*>(Blob2), 
            NumElements, 
            Md.threshold_kind, Md.norm, Md.threshold, Res);
        break;

      case BuiltinDType::U64:
        mneme::dev::compare_builtin_kernel<<<NumBlocks, NUM_THREADS_PER_BLOCK>>>(reinterpret_cast<const uint64_t*>(Blob1), 
            reinterpret_cast<const uint64_t*>(Blob2), 
            NumElements, 
            Md.threshold_kind, Md.norm, Md.threshold, Res);
        break;

      case BuiltinDType::I64:
        mneme::dev::compare_builtin_kernel<<<NumBlocks, NUM_THREADS_PER_BLOCK>>>(reinterpret_cast<const int64_t*>(Blob1), 
            reinterpret_cast<const int64_t*>(Blob2), 
            NumElements, 
            Md.threshold_kind, Md.norm, Md.threshold, Res);
        break;

      case BuiltinDType::F16:
        mneme::dev::compare_builtin_kernel<<<NumBlocks, NUM_THREADS_PER_BLOCK>>>(reinterpret_cast<const __half*>(Blob1), 
            reinterpret_cast<const __half*>(Blob2), 
            NumElements, 
            Md.threshold_kind, Md.norm, Md.threshold, Res);
        break;

      case BuiltinDType::F32:
        mneme::dev::compare_builtin_kernel<<<NumBlocks, NUM_THREADS_PER_BLOCK>>>(reinterpret_cast<const float*>(Blob1), 
            reinterpret_cast<const float*>(Blob2), 
            NumElements, 
            Md.threshold_kind, Md.norm, Md.threshold, Res);
        break;

      case BuiltinDType::F64:
        mneme::dev::compare_builtin_kernel<<<NumBlocks, NUM_THREADS_PER_BLOCK>>>(reinterpret_cast<const double*>(Blob1), 
            reinterpret_cast<const double*>(Blob2), 
            NumElements, 
            Md.threshold_kind, Md.norm, Md.threshold, Res);
        break;
    ...
    EC = DeviceVendorTraits::DeviceErrorCheck(
        DeviceVendorTraits::DeviceFree(Res));
