...
def tune(executor, record_db, record_id):
    ...
    print(
        f"Best config has optimization pipeline {best[0].passes}, specialize: {best[0].specialize} and specilize_dims: {best[0].specialize_dims} and set launch bounds: {best[0].set_launch_bounds} shows speedup over base line : {speedup} and total time: {avg_time}"
    )
