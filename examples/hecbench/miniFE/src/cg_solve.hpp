  ...
  template<typename OperatorType,
    ...
    typename Matvec>
          ...
          timer_type* my_cg_times)
      {
        ...
        DEVICE_CHECK(Device::deviceMalloc((void**)&d_r, sizeof(MINIFE_SCALAR) * r.coefs.size()));
        DEVICE_CHECK(Device::deviceMalloc((void**)&d_p, sizeof(MINIFE_SCALAR) * p.coefs.size()));
        DEVICE_CHECK(Device::deviceMalloc((void**)&d_Ap, sizeof(MINIFE_SCALAR) * Ap.coefs.size()));
        DEVICE_CHECK(Device::deviceMalloc((void**)&d_x, sizeof(MINIFE_SCALAR) * x.coefs.size()));
        DEVICE_CHECK(Device::deviceMalloc((void**)&d_b, sizeof(MINIFE_SCALAR) * b.coefs.size()));
        DEVICE_CHECK(Device::deviceMalloc((void**)&d_Arowoffsets, sizeof(LocalOrdinalType) * A.row_offsets.size()));
        DEVICE_CHECK(Device::deviceMalloc((void**)&d_Acols, sizeof(GlobalOrdinalType) * A.packed_cols.size()));
        DEVICE_CHECK(Device::deviceMalloc((void**)&d_Acoefs, sizeof(MINIFE_SCALAR) * A.packed_coefs.size()));
        DEVICE_CHECK(Device::deviceCopy(d_r, r_ptr, sizeof(MINIFE_SCALAR) * r.coefs.size(), Device::memcpyHostToDeviceKind()));
        DEVICE_CHECK(Device::deviceCopy(d_p, p_ptr, sizeof(MINIFE_SCALAR) * p.coefs.size(), Device::memcpyHostToDeviceKind()));
        DEVICE_CHECK(Device::deviceCopy(d_Ap, Ap_ptr, sizeof(MINIFE_SCALAR) * Ap.coefs.size(), Device::memcpyHostToDeviceKind()));
        DEVICE_CHECK(Device::deviceCopy(d_x, x_ptr, sizeof(MINIFE_SCALAR) * x.coefs.size(), Device::memcpyHostToDeviceKind()));
        DEVICE_CHECK(Device::deviceCopy(d_b, b_ptr, sizeof(MINIFE_SCALAR) * b.coefs.size(), Device::memcpyHostToDeviceKind()));
        DEVICE_CHECK(Device::deviceCopy(d_Arowoffsets, Arowoffsets, sizeof(LocalOrdinalType) * A.row_offsets.size(), Device::memcpyHostToDeviceKind()));
        DEVICE_CHECK(Device::deviceCopy(d_Acols, Acols, sizeof(GlobalOrdinalType) * A.packed_cols.size(), Device::memcpyHostToDeviceKind()));
        DEVICE_CHECK(Device::deviceCopy(d_Acoefs, Acoefs, sizeof(MINIFE_SCALAR) * A.packed_coefs.size(), Device::memcpyHostToDeviceKind()));
        ...
        DEVICE_CHECK(Device::deviceCopy(x_ptr, d_x, sizeof(MINIFE_SCALAR) * x.coefs.size(), Device::memcpyDeviceToHostKind()));
        ...
        DEVICE_CHECK(Device::deviceFree(d_r));
        DEVICE_CHECK(Device::deviceFree(d_p));
        DEVICE_CHECK(Device::deviceFree(d_Ap));
        DEVICE_CHECK(Device::deviceFree(d_x));
        DEVICE_CHECK(Device::deviceFree(d_b));
        DEVICE_CHECK(Device::deviceFree(d_Arowoffsets));
        DEVICE_CHECK(Device::deviceFree(d_Acols));
        DEVICE_CHECK(Device::deviceFree(d_Acoefs));
      }
