...
#pragma omp atomic
  ...
  template<typename VectorType>
        ...
        typename VectorType::ScalarType *d_wcoefs)
    {
      ...
      if(beta == 0.0) {
        if(alpha == 1.0) {
          wx_kernel<VectorType><<<dim3(grids), dim3(threads)>>>(n, d_xcoefs, d_wcoefs);
        } else {
          wax_kernel<VectorType><<<dim3(grids), dim3(threads)>>>(n, alpha, d_xcoefs, d_wcoefs);
        }
      } else {
        if(alpha == 1.0) {
          wxby_kernel<VectorType><<<dim3(grids), dim3(threads)>>>(n, d_xcoefs, beta, d_ycoefs, d_wcoefs);
        } else {
          waxby_kernel<VectorType><<<dim3(grids), dim3(threads)>>>(n, alpha, d_xcoefs, beta, d_ycoefs, d_wcoefs);
        }
      }
  ...
  template<typename VectorType>
        ...
        MINIFE_SCALAR *d_ycoefs)
    {
      ...
      if(alpha == 1.0 && beta == 1.0) {
        dyx_kernel<VectorType><<<dim3(grids), dim3(threads)>>>(n, d_xcoefs, d_ycoefs);
      } else if (beta == 1.0) {
        dyax_kernel<VectorType><<<dim3(grids), dim3(threads)>>>(n, alpha, d_xcoefs, d_ycoefs);
      } else if (alpha == 1.0) {
        yxby_kernel<VectorType><<<dim3(grids), dim3(threads)>>>(n, d_xcoefs, beta, d_ycoefs);
      } else if (beta == 0.0) {
        wax_kernel<VectorType><<<dim3(grids), dim3(threads)>>>(n, alpha, d_xcoefs, d_ycoefs);  // reuse the "wax" kernel
      } else {
        yaxby_kernel<VectorType><<<dim3(grids), dim3(threads)>>>(n, alpha, d_xcoefs, beta, d_ycoefs);
      }
...
#pragma unroll
  ...
  template<typename Vector>
        ...
        typename Vector::ScalarType *d_ycoefs)
    {
      ...
      Scalar* d;
      DEVICE_CHECK(Device::deviceMalloc((void**)&d, sizeof(Scalar)*1024));
      DEVICE_CHECK(Device::deviceMemset(d, 0, sizeof(Scalar)*1024));
      dot_kernel<Scalar><<<dim3(grids), dim3(threads)>>>(n, d_xcoefs, d_ycoefs, d);
      final_reduce<Scalar><<<dim3(1), dim3(256)>>>(d);
      DEVICE_CHECK(Device::deviceCopy((void**)&result, d, sizeof(Scalar), Device::memcpyDeviceToHostKind()));
      DEVICE_CHECK(Device::deviceFree(d));
  ...
  template<typename Vector>
    ...
    dot_r2(const Vector& x, const typename Vector::ScalarType *d_xcoefs)
    {
      ...
      DEVICE_CHECK(Device::deviceMalloc((void**)&d, sizeof(Scalar)*1024));
      DEVICE_CHECK(Device::deviceMemset(d, 0, sizeof(Scalar)*1024));
      dot_kernel<Scalar><<<dim3(grids), dim3(threads)>>>(n, d_xcoefs, d_xcoefs, d);
      final_reduce<Scalar><<<dim3(1), dim3(256)>>>(d);
      DEVICE_CHECK(Device::deviceCopy(&result, d, sizeof(Scalar), Device::memcpyDeviceToHostKind()));
      DEVICE_CHECK(Device::deviceFree(d));
